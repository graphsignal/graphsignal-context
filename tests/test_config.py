"""Tests for graphsignal_context.config."""
...
def isolated_config(monkeypatch):
    ...
    import graphsignal_context.config as config_mod
...
def test_load_config_missing_returns_empty_dict():
    from graphsignal_context.config import load_config
    assert load_config() == {}


def test_save_and_load_config():
    from graphsignal_context.config import save_config, load_config
    save_config({"api_key": "test-key", "other": "value"})
    assert load_config() == {"api_key": "test-key", "other": "value"}


def test_get_api_key_from_config():
    from graphsignal_context.config import set_api_key, get_api_key
    set_api_key("stored-key")
    assert get_api_key() == "stored-key"


def test_get_api_key_from_env(monkeypatch):
    from graphsignal_context.config import get_api_key
...
def test_set_api_key_persists():
    from graphsignal_context.config import set_api_key, get_api_key, load_config
