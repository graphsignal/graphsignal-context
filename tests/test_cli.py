"""Tests for graphsignal_debug CLI."""
...
from graphsignal_debug.cli import cli
...
def test_fetch_requires_login():
    with patch("graphsignal_debug.cli.get_api_key", return_value=None):
        result = runner.invoke(cli, ["fetch", "--start", "2026-03-10T00:00:00Z", "--end", "2026-03-12T00:00:00Z"])
    assert result.exit_code != 0
    assert "Not logged in" in result.output or "graphsignal-debug login" in result.output


def test_fetch_success_outputs_context():
    with patch("graphsignal_debug.cli.get_api_key", return_value="test-key"):
        with patch("graphsignal_debug.cli.fetch_debug_context", return_value="context output"):
            result = runner.invoke(cli, ["fetch", "--start", "2026-03-10T00:00:00Z", "--end", "2026-03-12T00:00:00Z"])
    assert result.exit_code == 0
...
def test_fetch_rejects_end_before_start():
    with patch("graphsignal_debug.cli.get_api_key", return_value="key"):
